...
export default function DisplayConfigPage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const [data, global, op, tt] = await Promise.all([
        api.listDisplays(),
        api.getConfig(),
        api.listOperators(),
        api.listTrainTypes(),
  ...
  const save = async () => {
    ...
    try {
      setSaving(true);
      await api.saveStationDisplayConfig(displayedStation.id, localConfig);
      await load();
    } catch (err: any) {
  ...
  return (
    <div className="h-screen overflow-hidden bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950 text-white">
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold">{displayedStation.short || displayedStation.name}</h1>
                <p className="text-xs text-slate-400">{displayedStation.name} · ID {displayedStation.id}</p>
              </div>
            </div>
          </div>
      ...
      <main className="h-[calc(100vh-81px)] max-w-[1600px] mx-auto px-4 sm:px-6 py-4 grid grid-cols-1 2xl:grid-cols-[460px_minmax(0,1fr)] gap-4 overflow-hidden">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white" value={displayedConfig?.station_name || ""} onChange={(e) => update({ station_name: e.target.value })} />
              </div>
              <div>
              ...
              <div>
                ...
                <select className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white" value={(displayedConfig?.language as Language) ?? "es"} onChange={(e) => update({ language: e.target.value as Language })}>
        ...
        <aside className="space-y-4 min-h-0 overflow-hidden flex flex-col">
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 flex flex-col min-h-0 flex-1">
            ...
            {trains.length === 0 ? (
              ...
              <div className="overflow-auto rounded-xl border border-white/10 min-h-0 flex-1">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {trains.map((train) => (
                      <tr key={train.id} className="hover:bg-white/5 transition">
                        ...
                        <td className="py-2.5 px-3 font-mono text-amber-300 font-semibold whitespace-nowrap">
                          {train.number}
                        </td>
                        <td className="py-2.5 px-3 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            {train.type_color ? (
                              <span
                                className="inline-flex min-w-14 items-center justify-center rounded px-2 py-1 text-xs font-bold text-white"
                            ...
                            <span className="text-xs text-slate-400 truncate max-w-[10rem]">
                              {train.type_name || ""}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="py-2.5 px-3 text-white min-w-[14rem]">
                        ...
                        <td className="py-2.5 px-3 whitespace-nowrap">
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-3 py-1.5 rounded bg-amber-500 hover:bg-amber-400 text-black text-xs font-semibold disabled:opacity-60"
                            >
                              Editar
                            </button>
                            <button
                              ...
                              className="px-3 py-1.5 rounded bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold disabled:opacity-60"
                            >
