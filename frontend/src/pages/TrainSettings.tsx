...
export default function TrainSettings() {
  ...
  const refresh = async () => {
    const [op, tt] = await Promise.all([api.listOperators(), api.listTrainTypes()]);
    setOperators(op);
    setTrainTypes(tt);
  };
    ...
    <div className="min-h-screen bg-board-bg text-board-ink p-8 font-body">
      <header className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-4 items-center">
          <a href="/trains" className="text-board-amber underline">
            Trenes
          </a>
        ...
        <section className="bg-board-row rounded-lg p-5">
          ...
          <Catalog
            items={operators.map((o) => ({
              id: o.id,
              label: o.name,
              extra: o.logo_url ? <img src={fileUrl(o.logo_url)!} className="h-6" onError={(e) => handleImgError(e, o.name)} /> : null,
            }))}
          ...
          {editingOperator && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setEditingOperator(null)}>
              <div
                className="bg-board-row rounded-lg p-6 w-full max-w-md mx-4 border border-board-amber"
                  ...
                  <div>
                    ...
                    {editingOperator.pre_announce_ogg && (
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <button
                          onClick={async () => {
                            await api.deleteOperatorPre(editingOperator.id);
                            refresh();
                          }}
                    ...
                    <input
                      type="file"
                      accept="audio/ogg,audio/opus,audio/mpeg,.ogg,.opus,.mp3"
                      onChange={(e) => setOperatorPre(e.target.files?.[0] ?? null)}
                    />
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={async () => {
                        ...
                        api.updateOperator(editingOperator.id, editingOperator.name, operatorLogo).then(() => {
                    ...
                    <button
                      onClick={() => {
                        api.deleteOperator(editingOperator.id).then(() => {
                          setEditingOperator(null);
                          setOperatorLogo(null);
                          setOperatorPre(null);
                          refresh();
          ...
          <Catalog
            items={trainTypes.map((t) => ({
              ...
              extra: t.logo_url ? (
                ...
                <span className="inline-block w-4 h-4 rounded" style={{ background: t.color }} />
              ),
            }))}
                  ...
                  <div>
                    ...
                    {editingType.logo_url && (
                      <img
                        ...
                        className="h-8 mb-2"
                        alt="Logo"
                        onError={(e) => handleImgError(e, editingType.code)}
                      />
                    )}
                  ...
                  <div>
                    ...
                    {editingType.pre_announce_ogg && (
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <button
                          onClick={async () => {
                            await api.deleteTrainTypePre(editingType.id);
                            refresh();
                          }}
                          className="text-board-red text-sm"
                          title="Eliminar audio"
                    ...
                    <input
                      type="file"
                      accept="audio/ogg,audio/opus,audio/mpeg,.ogg,.opus,.mp3"
                      onChange={(e) => setTypePre(e.target.files?.[0] ?? null)}
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-board-dim uppercase tracking-wider mb-1.5">
                    ...
                    />
                    <p className="text-[11px] text-board-dim mt-1">
                      Variables: {"{type_name}"}, {"{number_text}"}, {"{destination}"}, {"{platform}"}, {"{sector_text}"}, {"{stops}"},{" "}
                      {"{origin}"}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => {
                        api.deleteTrainType(editingType.id).then(() => {
                          setEditingType(null);
                          setTypeLogo(null);
                          setTypePre(null);
                          refresh();
        ...
        {items.map((it) => (
          <li key={it.id} className="flex items-center justify-between py-1 border-b border-white/5">
            ...
            <div className="flex gap-1">
              {onEdit && (
                <button onClick={() => onEdit(it.id)} className="text-board-amber text-sm">
                  Editar
                </button>
              )}
              <button onClick={() => onRemove(it.id)} className="text-board-red text-sm">
                ✕
              </button>
            </div>
          </li>
